...
def pre_model(backbone, weights_decay=float(0)):
    ...
    if weights_decay!=float(0):
        clf = Dense(2, use_bias=False, kernel_regularizer=tf.keras.regularizers.l2(weights_decay))(x)
    else:
        clf = Dense(2, use_bias=False)(x)
...
def test_fn(model, image, label):
    model_output = model([image, label], training=False)
    # focal loss
    loss_p = sfl_p(y_true=label[:, 0], y_pred=model_output["logits_p"])
    loss_m = sfl_m(y_true=label[:, 1], y_pred=model_output["logits_m"])
    loss_g = sfl_g(y_true=label[:, 2], y_pred=model_output["logits_g"])
...
def main(model, val_fold):
    ...
    while epochs <= max_epochs:
        ...
        for i in tqdm(range(train_total_batch)):
            ...
            train_output = train_fn(model, imgs, labels, epochs, lr)
            #
        ...
        for j in tqdm(range(val_total_batch)):
            ...
            val_output = test_fn(model, imgs, labels)
            #
        ...
        # metrics
        # wandb.log({"Train Malignant Precision": tr_pre, "Validation Malignant Precision": val_pre}, step=epochs)
        # wandb.log({"Train Malignant Recall": tr_re, "Validation Malignant Recall": val_re}, step=epochs)
        # wandb.log({"Train Malignant F1": tr_f1, "Validation Malignant F1": val_f1}, step=epochs)
        # AUC
        ...
        # pr_re_plot = utils_vis.train_val_pr_plot(y_train, y_val, average='micro', show=False)
        # wandb.log({"Precision-recall curve": wandb.Image(pr_re_plot)}, step=epochs)
        # Embedding plot-------------------------------------
        ...
        if val_auc_m>current_auc:
            ...
            model.save_weights(os.path.join(weights_dir, 'best_auc{}.h5'.format(epochs)))
        ...
        if val_acc_sub[1]>current_acc:
            ...
            model.save_weights(os.path.join(weights_dir, 'best_acc{}.h5'.format(epochs)))
            wandb.save(os.path.join(weights_dir, 'best_acc{}.h5'.format(epochs)), base_path=base_path)
        if p_at_1>current_p:
            current_p=p_at_1
            best_p_file = glob.glob(weights_dir+'best_p@{}*'.format(k))
            for file in best_p_file:
                best_p_path = weights_dir + file
                if os.path.exists(best_p_path):
                    os.remove(best_p_path)
            model.save_weights(os.path.join(weights_dir, 'best_p@{}_{}.h5'.format(k, epochs)))
        ...
        if map>current_map:
            ...
            model.save_weights(os.path.join(weights_dir, 'best_map{}.h5'.format(epochs)))
            wandb.save(os.path.join(weights_dir, 'best_map{}.h5'.format(epochs)), base_path=base_path)
        if epochs==max_epochs:
            model.save_weights(os.path.join(weights_dir, 'checkpoint_{}.h5'.format(epochs)))
        ...
        print()
        # early stopping
        if epochs > 11 and (val_total_loss - train_total_loss) > 5:
            counter += 1
        else:
            counter=0
        if counter == patience:
...
if __name__ == '__main__':
    ...
    main(model, val_fold=config.val_fold)
    sys.exit(0)
